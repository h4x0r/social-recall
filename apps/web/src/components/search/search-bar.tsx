  ...
  return (
    <div className="relative">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <SearchHint label="Security experts" />
        <SearchHint label="Ex-Google" />
        <SearchHint label="Founders" />
        <SearchHint label="Investors" />
      </div>
    </div>
  );
}
...
function SearchHint({ label }: { label: string }) {
  return (
    <button className="rounded-full border border-border bg-card px-3 py-1 text-xs text-muted-foreground transition-all hover:border-primary hover:text-foreground">
      {label}
    </button>
  );
}
