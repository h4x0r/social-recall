// Supabase Edge Function: infer-skills
// Calls Claude API to extract skills from LinkedIn profile data
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profile || !profile.name || !profile.headline) {
      return new Response(
        JSON.stringify({ error: 'Missing required profile data (name, headline)' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
